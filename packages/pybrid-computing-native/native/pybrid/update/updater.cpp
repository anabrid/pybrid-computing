...
std::vector<uint8_t> hex2bin(const std::vector<uint8_t>& hex_data) {
    ...
    for (size_t hex_idx = 0; hex_idx < hex_data.size();) {
        ...
        if (kind == Eof) {
            ...
            if (current_addr < FW_BASE_ADDR) return {};
            uint32_t full_addr = current_addr + addr - FW_BASE_ADDR;
            if (full_addr + bytes > result.size()) result.resize(full_addr + bytes);
 ...
 * OTAUpdater
 */
OTAUpdater::UpdateResult OTAUpdater::execute(string firmware_path, const vector<string>& targets, uint16_t port) {
    vector<int> indices(targets.size());
    iota(indices.begin(), indices.end(), 0);

    auto firmware = read_firmware(firmware_path);
    if (firmware.is_err()) return UpdateResult::err(firmware.err_value());

    m_firmware_bin.clear();
    m_firmware_bin = hex2bin(firmware.ok_value());
    if (m_firmware_bin.empty()) return UpdateResult::err("Unable to convert firmware to binary data!");
    ...
    auto connect_futures = indices | views::transform([&](int index) { async(fan_connect, index); });
    for (auto& fut : connect_futures)
        fut.get();

    if (!is_err) return UpdateResult::err("Unable to connect to all targets.");

    return UpdateResult::ok(true);
}

OTAUpdater::ReadResult OTAUpdater::read_firmware(const string& firmware_path) {
    // load firmware and convert to bianry
    if (!exists(firmware_path)) return ReadResult::err_fmt("Firmware not found at %s", firmware_path);

    vector<uint8_t> firmware_hex;
    basic_ifstream<uint8_t> firmware_in(firmware_path);
    if (!firmware_in.good()) return ReadResult::err_fmt("Unable to read firmware file %s", firmware_path);
...
OTAUpdater::TransportResult OTAUpdater::connect_to(const string& host, uint16_t port) {
    auto transport = make_unique<TCPTransport>();
    if (!transport->connect(host, port))
        return OTAUpdater::UpdateResult::err_fmt("Unable to connect to %s:%d", host, port);
}

OTAUpdater::UpdateResult OTAUpdater::upload(unique_ptr<TCPTransport>& transport) {
    // UpdateCommand cmd;
...
OTAUpdater::UpdateResult OTAUpdater::commit(unique_ptr<TCPTransport>& transport) {
    // TODO
}
