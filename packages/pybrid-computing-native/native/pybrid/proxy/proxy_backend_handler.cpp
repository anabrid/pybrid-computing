...
BackendDevice& BackendDevice::operator=(BackendDevice&& other) noexcept {
    if (this != &other) {
        ...
        location_stack = std::move(other.location_stack);
        location_carrier = std::move(other.location_carrier);
        health.store(other.health.load(std::memory_order_acquire), std::memory_order_release);
    }
    return *this;
}

ProxyBackendHandler::ProxyBackendHandler(std::mutex* log_mutex) : log_mutex_(log_mutex) {}
ProxyBackendHandler::~ProxyBackendHandler() = default;

void ProxyBackendHandler::add_backend(
    ...
    data_channel->set_control_response_callback(
        [control_ptr](std::vector<uint8_t> data) { control_ptr->on_tcp_response(std::move(data)); });

    data_channel->start();
    std::future<void> init_future = std::async(std::launch::deferred, []() { /* no-op: start() already completed */ });

    {
        ...
        if (stack.has_value() && carrier.has_value()) {
            for (auto& item : *dev.cached_module.mutable_items()) {
                if (item.has_entity_specification()) {
                    auto* loc = item.mutable_entity_specification()->mutable_entity()->mutable_location_v0();
...
void ProxyBackendHandler::start(RunCoordinator& run_coord, ErrorToClient error_cb) {
    ...
    for (auto& backend : backends_) {
        ...
        ctrl->register_callback(pb::MessageV1::kErrorMessageFieldNumber, [this](pb::MessageV1& msg) {
            if (!running_.load(std::memory_order_acquire)) return;
            if (debug_) {
                std::cerr << "[ProxyServer] DEBUG: Error from device: " << msg.error_message().description() << "\n";
            }
    ...
    for (auto* backend : snapshot) {
        if (backend->data_channel_init_future.valid()) {
            backend->data_channel_init_future.wait_for(
                std::chrono::duration<double>(BACKEND_UDP_NEGOTIATION_TIMEOUT_SECS + 1.0));
        }
    }
...
BackendDevice* ProxyBackendHandler::find_backend_for_path(const std::string& entity_path) {
    ...
    std::string mac = (slash_pos != std::string::npos) ? entity_path.substr(start, slash_pos - start)
...
BroadcastResult ProxyBackendHandler::broadcast_to_backends(
    ...
    for (auto* backend : targets) {
        ...
        tasks.push_back({backend, std::async(std::launch::async, [ctrl, r = std::move(req), timeout_secs]() mutable {
    ...
    for (auto& task : tasks) {
        try {
            ...
            if (include_responses) result.responses.emplace_back(resp);
...
bool ProxyBackendHandler::all_backends_healthy() const {
    std::lock_guard<std::mutex> lock(backends_mutex_);
    for (auto const& backend : backends_) {
        if (backend.health.load(std::memory_order_acquire) != BackendHealth::HEALTHY) {
            return false;
        }
    }
    return true;
...
void ProxyBackendHandler::set_backend_health(BackendDevice& backend, BackendHealth h) {
    backend.health.store(h, std::memory_order_release);
}

void ProxyBackendHandler::set_backend_health_for_test(size_t index, int new_health) {
    std::lock_guard<std::mutex> lock(backends_mutex_);
    if (index >= backends_.size()) {
        throw std::out_of_range("backend index out of range");
    }
    set_backend_health(backends_[index], static_cast<BackendHealth>(new_health));
}
...
int ProxyBackendHandler::get_backend_health(size_t index) const {
    ...
    return static_cast<int>(backends_[index].health.load(std::memory_order_acquire));
}
...
bool ProxyBackendHandler::reconnect_backend(BackendDevice& backend, std::chrono::milliseconds timeout) {
    if (log_mutex_) {
        std::lock_guard<std::mutex> lock(*log_mutex_);
        std::cerr << "[ProxyServer] Connection lost to " << backend.host << ":" << backend.port
                  << ", reconnecting...\n";
    }
    try {
        if (!backend.control) return false;
        bool ok = backend.control->reconnect(std::chrono::milliseconds{500}, std::make_optional(timeout));
        if (!ok) {
            if (log_mutex_) {
                std::lock_guard<std::mutex> lock(*log_mutex_);
                std::cerr << "[ProxyServer] Failed to reconnect to " << backend.host << ":" << backend.port << "\n";
            }
            return false;
        }
        if (backend.data_channel) {
        ...
        pb::Module module = backend.control->extract(
            ...
            /*calibration=*/false,
            BACKEND_REQUEST_TIMEOUT_SECS);
        if (backend.location_stack.has_value() && backend.location_carrier.has_value()) {
        ...
        if (log_mutex_) {
            std::lock_guard<std::mutex> lock(*log_mutex_);
            std::cerr << "[ProxyServer] Reconnected to " << backend.host << ":" << backend.port << "\n";
        }
        return true;
    } catch (const std::exception& e) {
        if (log_mutex_) {
            std::lock_guard<std::mutex> lock(*log_mutex_);
            std::cerr << "[ProxyServer] Reconnect to " << backend.host << ":" << backend.port << " failed: " << e.what()
                      << "\n";
        }
        return false;
    }
...
void ProxyBackendHandler::install_forward_callbacks() {
    ...
    for (auto* backend_ptr : targets()) {
        ...
        auto forward_lambda = [this, weak_session, backend_ptr](pb::MessageV1& msg) {
            ...
            if (msg.has_run_state_change_message()) {
                ...
                if (!backend_ptr->carrier_paths.empty()) {
                    msg.mutable_run_state_change_message()->mutable_entity()->set_path(
                        "/" + backend_ptr->carrier_paths[0]);
                }
                ...
                if (new_state == pb::ERROR) {
                    std::string desc = "Backend device error from " + backend_ptr->host + ":" +
                ...
                if (new_state == pb::DONE) {
                    if (backend_ptr->data_channel) {
                        ...
                        if (udp_st && log_mutex_) {
                            std::lock_guard<std::mutex> lk(*log_mutex_);
                            std::cerr << "[ProxyServer] Backend " << backend_ptr->host << ":" << backend_ptr->port
                                      << " run stats: " << udp_st->packets_received << " packets received, "
                                      << udp_st->packets_dropped << " dropped, " << udp_st->bytes_received
                                      << " bytes received, " << udp_st->queue_size << " queued\n";
                        }
                    ...
                    if (run_coordinator_ && run_coordinator_->on_done()) {
                        ...
                        if (sess->transport()) {
                            ...
                            if (log_mutex_) {
                                std::lock_guard<std::mutex> lk(*log_mutex_);
                                std::cerr << "[ProxyServer] Client TCP stats: " << tcp_st.messages_sent
                                          << " messages sent, " << tcp_st.bytes_sent << " bytes sent, "
                                          << tcp_st.messages_received << " messages received, " << tcp_st.bytes_received
                                          << " bytes received\n";
                            }
        ...
        if (backend.control) {
            backend.control->register_callback(pb::MessageV1::kRunStateChangeMessageFieldNumber, forward_lambda);
        }
    }
}
...
void ProxyBackendHandler::clear_forward_callbacks() {
    for (auto* backend : targets()) {
        ...
        if (backend->control) {
            backend->control->unregister_callback(pb::MessageV1::kRunStateChangeMessageFieldNumber);
        }
    }
}
