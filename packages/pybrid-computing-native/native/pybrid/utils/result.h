...
class Result {
    ...
    explicit Result(std::variant<V, E> result) : result(std::move(result)) { err_unused = is_err(); }

public:
    Result(V ok_value) : result(std::variant<V, E>(std::in_place_index<0>, std::move(ok_value))) {}

    template <typename O, std::enable_if_t<!std::is_same_v<V, O>, int> = 0>
    Result(Result<O, E>&& other_result) : result(std::variant<V, E>(std::in_place_index<1>, "")) {
        assert(other_result.is_err());
        result = std::move(other_result.err_value());
    }

    Result(Result&& other_result) : result(std::move(other_result.result)) {
        err_unused = other_result.err_unused;
        other_result.err_unused = false;
    }

    ~Result() { assert(!err_unused); }
    ...
    static std::enable_if_t<std::is_same_v<E, std::string>, Result<V, E>> err_concat(Args&&... args) {
    ...
    template <typename... Args>
    static std::enable_if_t<std::is_same_v<E, std::string>, Result<V, E>> err_fmt(const char* format, Args... args) {
    ...
    [[nodiscard]] bool is_ok() const { return result.index() == 0; }
    ...
    [[nodiscard]] V& ok_value() { return std::get<0>(result); }
