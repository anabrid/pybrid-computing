...
from pybrid.lucipy.circuits import Circuit, Constant, Identity, Input, Integrator, Multiplier, Output
...
class TestIntegratorAllocation:
    ...
    def test_allocate_integrator(self):
        ...
        assert isinstance(i0, Integrator), "int() must return an Integrator instance"
        assert 0 <= i0.id <= 7, f"Integrator id must be in range 0-7, got {i0.id}"
        assert i0.lane == i0.id, (
            f"Integrator lane must equal its id (M0 block offset=0), " f"got lane={i0.lane} for id={i0.id}"
        )
    ...
    def test_allocate_integrator_with_ic(self):
        ...
        assert isinstance(i0, Integrator), "int(ic=0.5) must return an Integrator instance"
        ...
        assert m0.elements[i0.id].computation.ic == pytest.approx(0.5), (
            f"Integrator {i0.id} IC should be 0.5, " f"got {m0.elements[i0.id].computation.ic}"
        )
    ...
    def test_allocate_all_integrators(self):
        ...
        assert len(set(ids)) == 8, f"All 8 integrator ids must be distinct, got {ids}"
        assert set(ids) == set(range(8)), f"Integrator ids must cover 0-7, got {sorted(ids)}"
...
class TestMultiplierAllocation:
    ...
    def test_allocate_multiplier(self):
        ...
        assert isinstance(m0, Multiplier), "mul() must return a Multiplier instance"
        assert 0 <= m0.id <= 3, f"Multiplier id must be in range 0-3, got {m0.id}"
        assert m0.lane == 8 + m0.id, (
            f"Multiplier lane must be 8 + id (M1 block offset), " f"got lane={m0.lane} for id={m0.id}"
        )
    ...
    def test_allocate_all_multipliers(self):
        ...
        assert len(set(ids)) == 4, f"All 4 multiplier ids must be distinct, got {ids}"
        assert set(ids) == set(range(4)), f"Multiplier ids must cover 0-3, got {sorted(ids)}"
...
class TestIdentityCreation:
    ...
    @pytest.mark.parametrize(
        "mul_idx,input_name,expected_offset",
        [
            (0, "a", 0),
            (0, "b", 1),
            (1, "a", 2),
    ...
    def test_identity_offset(self, mul_idx, input_name, expected_offset):
        ...
        assert isinstance(result, Identity), f"mul{mul_idx}.{input_name}.id() must return an Identity instance"
        assert result.offset == expected_offset, (
            f"mul{mul_idx}.{input_name} identity offset must be {expected_offset}, " f"got {result.offset}"
        )
    ...
    def test_id_carries_circuit_id(self):
        ...
        assert id_a._circuit_id == circuit._circuit_id, "Identity._circuit_id should match its circuit"
    ...
    def test_id_no_allocation_conflict_with_mul(self):
        ...
        assert id0.offset == 0, "Identity offset=0 must work when multiplier slot 0 is occupied"
    ...
    def test_id_same_input_multiple_times(self):
        ...
        assert id_a1 == id_a2, "Repeated m0.a.id() calls must return equivalent Identity elements"
...
class TestConstantAllocation:
    ...
    def test_allocate_constant(self):
        ...
        assert isinstance(c0, Constant), "const() must return a Constant instance"
        assert c0.id == 0, f"First constant id should be 0, got {c0.id}"
    ...
    def test_allocate_two_constants(self):
        ...
        assert c0.id != c1.id, f"Two constants must have different ids, " f"got c0.id={c0.id}, c1.id={c1.id}"
...
class TestInputOutputAllocation:
    ...
    def test_allocate_acl_port(self, method, expected_type):
        ...
        assert isinstance(result, expected_type), f"{method}() must return a {expected_type.__name__} instance"
        assert result.port == 0, f"First {method} port should be 0 (greedy), got {result.port}"
        assert 24 <= result.lane <= 31, f"{method} lane must be in 24-31 (ACL range), got {result.lane}"
