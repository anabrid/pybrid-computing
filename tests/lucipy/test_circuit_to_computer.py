...
class TestToComputer:
    ...
    def test_to_computer_harmonic_oscillator(self):
        ...
        assert cluster.m0block.elements[i0.id].computation.ic == pytest.approx(
            1.0
        ), f"Integrator {i0.id} IC should be 1.0"
        assert cluster.m0block.elements[i1.id].computation.ic == pytest.approx(
            0.0
        ), f"Integrator {i1.id} IC should be 0.0"

        # Find lanes for connection i0 -> i1 (weight +1.0)
        lanes_i0_to_i1 = [lane for lane in range(24) if cluster.ublock.outputs[lane] == i0.lane]
        assert len(lanes_i0_to_i1) >= 1, "Expected at least 1 lane connecting i0 -> i1"
        ...
        for lane in lanes_i0_to_i1:
            ...
            assert effective == pytest.approx(1.0), (
                f"Coefficient for i0->i1 at lane {lane} should be 1.0, " f"got effective={effective}"
            )
            # IBlock should route to i1
            assert lane in cluster.iblock.outputs[i1.lane], (
                f"IBlock outputs[{i1.lane}] should contain lane {lane} " f"for i0->i1 connection"
            )

        # Find lanes for connection i1 -> i0 (weight -1.0)
        lanes_i1_to_i0 = [lane for lane in range(24) if cluster.ublock.outputs[lane] == i1.lane]
        assert len(lanes_i1_to_i0) >= 1, "Expected at least 1 lane connecting i1 -> i0"
        ...
        for lane in lanes_i1_to_i0:
            ...
            effective = c_factor * 8 if upscaled else c_factor
            assert effective == pytest.approx(-1.0), (
                f"Coefficient for i1->i0 at lane {lane} should be -1.0, " f"got effective={effective}"
            )
            # IBlock should route to i0
            assert lane in cluster.iblock.outputs[i0.lane], (
                f"IBlock outputs[{i0.lane}] should contain lane {lane} " f"for i1->i0 connection"
            )
    ...
    def test_to_computer_warns_on_manual_changes(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            relevant_warnings = [x for x in w if issubclass(x.category, (UserWarning, DeprecationWarning))]
...
class TestDeepCopyLUCIDAC:
    ...
    def test_deepcopy_lucidac(self):
        ...
        # Basic type check
        assert isinstance(cloned, LUCIDAC), "deepcopy of LUCIDAC must produce a LUCIDAC instance"
        ...
        assert original_cluster.m0block.elements[0].computation.ic == pytest.approx(
            original_ic
        ...
        assert (
            original_cluster.ublock.outputs == original_u
