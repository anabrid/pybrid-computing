...
# currently only used for testing purposes, will eventually be implemented
# one we use more than one protocol version
...
class ProtoVersioning:
    ...
    def is_same(cls, lhs: pb.Version, rhs: pb.Version):
        """Compares thwo versions anc checks whether they are identical"""
    ...
    def is_recent(cls, input: pb.File | pb.Envelope):
        ...
        if not cls.is_versioned(input):
            raise Exception("Can only detect the version of Fiel/Envelope messages!")
    ...
    def update(cls, input: pb.File | pb.Envelope):
        ...
        while(cls.is_newer(cls.current(), cur.version)):
            ...
            # retrieve the mathing converter for the current type/version
