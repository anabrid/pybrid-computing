...
Integrator =    namedtuple("Integrator", ["id", "out", "a"])          #: Integrator, integrates input "a", output to "out"
Multiplier =    namedtuple("Multiplier", ["id", "out", "a", "b"])     #: Multiplier, multiplies inputs "a" with "b", output to "out"
Identity  =     namedtuple("Identity",  ["id", "out", "a"])          #: Identity element, just passes input "a" to output "out"
Constant =      namedtuple("Constant", ["id", "out" ])          #: Constant giver. output on (cross lane "out")
...
class DefaultLUCIDAC:
    ...
    def reservoir(default_value=False):
        return {
            Integrator: [default_value]*DefaultLUCIDAC.num_int,
            Multiplier: [default_value]*DefaultLUCIDAC.num_mul,
            Constant:   [default_value]*DefaultLUCIDAC.num_const,
            Front:      [default_value]*DefaultLUCIDAC.num_acls,
            Identity:   [default_value]*DefaultLUCIDAC.num_id,
        }
    
    @classmethod
    def make(cls, element:Element, idx):
        """
        ...
        """
        if element == Integrator:
            return Integrator(idx, cls.MIntOffset+idx, cls.MIntOffset+idx)
        ...
        if element == Identity:
            ...
            return Identity(idx, cls.MMulOffset+4+idx, cls.MMulOffset+idx)
        if element == Constant:
            ...
            return Constant(idx, 14 + idx)
        if element == Front:
            return Front(idx, cls.acl_offset + idx)
        raise ValueError(f"Illegal value for {element=}. Need a type like Int/Mul/Id/Const/Front from lucipy.circuits")
    ...
    def resolve_mout(cls, idx, reservoir):
        ...
        if idx < 8:  return reservoir[Integrator][idx]
        if idx < 12: return reservoir[Multiplier][idx - cls.MMulOffset]
        if idx < 16: return reservoir[Identity][idx - 12]
...
class Route:
    ...
    def resolve(self):
        ...
        """
        if isinstance(self.uin, get_args(Element)):
                self.uin = self.uin.out
        if isinstance(self.iout, get_args(Element)):
...
class MIntBlockState:
    ...
    def set_ic(self, el:Union[Integrator,int], val:float):
        if val < -1 or val > +1:
        ...
        el = el.id if isinstance(el, Integrator) else el
        self.ics[el] = val
    
    def set_k0(self, el:Union[Integrator,int], val:float):
        el = el.id if isinstance(el, Integrator) else el
        self.k0s[el] = val
    
    def set_k0_slow(self, el:Union[Integrator,int], val:bool):
...
class Probes:
    ...
    def measure(self, source:Union[Element,int], adc_channel=None):
        """
        ...
        uin = source.out if isinstance(source, get_args(Element)) else source
...
class Routing(Probes):
    ...
    def probe(self, source:Union[Element,int], front_port=None, weight=1):
        """
    ...
    def connect(self, source:Union[Element,int], target:Union[Element,int], weight:float=1):
        """
    ...
    def sanity_check(self, also_print=True):
        ...
        multipliers = [ DefaultLUCIDAC.make(Multiplier, i) for i in range(DefaultLUCIDAC.num_mul) ]
...
class Circuit(MIntBlockState, Routing):
    ...
    def alloc(self, element:Element, id=None):
        ...
        """
        try:
            idx = id if id != None else next_free(self.reservoir[element])
            if idx == None:
                raise ValueError(
                    f"No more free Computing Elements for type {element}, all {len(self.reservoir[element])} occupied!"
                    if id != None else
                    f"Compute Element {element} number {id} is already allocated")
            self.reservoir[element][idx] = True
            return DefaultLUCIDAC.make(element, idx)
        except KeyError:
            raise TypeError(f"Computing Element Type {element} not supported. Valid ones are {', '.join(map(str, self.reservoir.keys()))}")
        except IndexError:
    ...
    def int(self, *, id=None, ic:float=0, slow=False):
        ...
        If you pass an id, allocate that particular integrator.
        """
        el = self.alloc(Integrator, id)
        self.set_ic(el, ic)
    ...
    def const(self, id=None):
        ...
        return self.alloc(Constant, id)
    
    def identity(self, id=None):
        "Allocates one Identity element"
        return self.alloc(Identity, id)
    ...
    def load(self, config_message):
        ...
        if "/M0" in config_cluster:
            MIntBlockState.load(self, config_cluster["/M0"])
