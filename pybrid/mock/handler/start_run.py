...
class StartRunHandler(BaseHandler):
    ...
    async def handle(
        self, cmd: pb.StartRunCommand, protocol: Protocol
        ...
        :param protocol: The protocol instance for sending notifications.
        ...
        task = asyncio.create_task(self._execute_run(cmd, protocol))
    ...
    async def _execute_run(self, cmd: pb.StartRunCommand, protocol: Protocol):
        """
        ...
        :param protocol: The protocol instance for sending messages.
        """
        run_id = cmd.run.id
        ...
        # Get channel count from stored ConfigCommand, not from DaqConfig
        ...
        for carrier_mac in self.server._carrier_macs:
            await self._send_carrier_samples(
                protocol, run_id, carrier_mac, num_channels, num_samples, sample_rate
            )
    ...
    async def _send_carrier_samples(
        ...
        for start in range(0, actual_samples, chunk_size):
            ...
            await protocol.send_message(Protocol.new_message(msg, id=None))
            logger.debug(
